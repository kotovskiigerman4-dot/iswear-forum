...
import { Terminal, AlertCircle } from "lucide-react"; // Вернул Terminal сюда
      ...
      <Card className="w-full max-w-md p-8 border-primary/50 shadow-[0_0_30px_rgba(0,255,159,0.1)] relative z-10">
        <div className="flex flex-col items-center mb-8">
          ...
          <div className="relative group mb-6 flex items-center justify-center">
            ...
            <img 
              ...
              alt="SYSTEM_SKULL" 
              className="relative w-24 h-24 md:w-32 md:h-32 object-contain filter drop-shadow-[0_0_15px_rgba(0,255,159,0.5)]"
            />
          </div>
        ...
        {error && (
          <div className="mb-6 p-3 border border-red-500 bg-red-500/10 text-red-500 text-xs flex items-center gap-2 rounded">
            ...
            <span>{typeof error === 'string' ? error : "AUTH_ERROR: CHECK_CREDENTIALS"}</span>
          </div>
        )}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {mode === "register" && (
            <>
              ...
              <div>
                <label className="text-[10px] text-primary uppercase">{leet("REASON")}</label>
                <Textarea 
                  value={applicationReason} 
                  onChange={e => setApplicationReason(e.target.value)} 
                  required 
                  placeholder="Extended response required..."
          ...
          {mode === "register" && (
            <div>
              <label className="text-[10px] text-muted-foreground uppercase">ICQ / JABBER</label>
              <Input 
                value={icq} 

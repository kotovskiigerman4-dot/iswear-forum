    ...
    <div className="min-h-screen flex items-center justify-center p-4 relative overflow-hidden bg-black">
      ...
      <Card className="w-full max-w-md p-8 border-primary/50 shadow-[0_0_30px_rgba(0,255,159,0.1)] relative z-10">
        <div className="flex flex-col items-center mb-8">
          <img src={skullUrl} alt="L0G0" className="w-24 h-24 object-contain mb-4 animate-pulse" />
        ...
        {error && (
          <div className="mb-6 p-3 border border-red-500 bg-red-500/10 text-red-500 text-xs flex items-center gap-2 rounded">
            ...
            <span>{typeof error === 'string' ? error : "AUTH_ERROR"}</span>
          </div>
        )}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {mode === "register" && (
            <>
              ...
              <div>
                ...
                <Textarea value={applicationReason} onChange={e => setApplicationReason(e.target.value)} required className="min-h-[80px]" />
              </div>
            </>
          )}
          <div>
          ...
          <Button type="submit" className="w-full mt-6" disabled={isLoggingIn || isRegistering}>
            {leet(mode === "login" ? "INITIALIZE" : "CREATE_ID")}
          </Button>
        </form>
      </Card>
    </div>

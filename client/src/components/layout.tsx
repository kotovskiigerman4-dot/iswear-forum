...
import { ShieldAlert, LogOut, User as UserIcon, Users, Search, Bell } from "lucide-react";
              ...
              <>
                <Link href="/notifications">
                  <a className="relative p-2 text-primary/60 hover:text-primary transition-colors cursor-pointer">
                    ...
                    {unreadCount > 0 && (
                      <span className="absolute top-1 right-1 bg-destructive text-[9px] font-bold px-1 min-w-[14px] h-[14px] flex items-center justify-center rounded-full animate-pulse">
                        {unreadCount}
                      </span>
                    )}
                  </a>
                ...
                {(user.role === "ADMIN" || user.role === "MODERATOR") && (
                  <Link href="/admin">
                    <Button variant="outline" className="gap-2 border-accent text-accent hidden sm:flex">
                ...
                <Button variant="ghost" className="text-destructive px-2" onClick={() => logout()}>
